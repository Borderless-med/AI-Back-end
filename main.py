...
def handle_chat(query: UserQuery):
    ...
    try:
        ranking_prompt = f"""
        Analyze the user's query to determine their priorities for choosing a dental clinic.
        ...
        For a general query about "quality", prioritize "sentiment_dentist_skill" and "sentiment_overall".
        For "convenience", prioritize "sentiment_convenience". For "value" or "price", prioritize "sentiment_cost_value".
        ...
        ranking_priority = ['sentiment_overall', 'rating'] # Default fallback ranking
    ...
    if candidate_clinics:
        if filters:
            for clinic in candidate_clinics:
                ...
                if filters.get('max_distance') and (clinic.get('distance') is None or clinic.get('distance', 999) > filters.get('max_distance')): match = False
    ...
    if final_candidates:
        # Add default tie-breakers to the dynamic priority list
        ranking_priority.extend(['rating', 'reviews'])
        # De-duplicate the priority list
        ranking_priority = list(dict.fromkeys(ranking_priority)) 
        
        print(f"Final ranking priority: {ranking_priority}")
    ...
    **FORMATTING RULE: You MUST use paragraphs to structure your response. Start with an intro. Present each clinic in its own paragraph with a bolded name. End with a summary.**
